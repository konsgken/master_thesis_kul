...
import resnet18_multi_task_learning_model as model
...
train_image_datasets = nyu_dataloader(trainingImageDir, center_train, train_lefttop_pixel, train_rightbottom_pixel,
...
test_image_datasets = nyu_dataloader(testingImageDir, center_test, test_lefttop_pixel, test_rightbottom_pixel,
...
def train():
    ...
    gesture_dataloader = torch.utils.data.DataLoader(gesture_image_dataset, batch_size=64, shuffle=True, num_workers=2)
    ntu_dataloader_iter = iter(gesture_dataloader)
    ...
    for epoch in range(nepoch):
        ...
        for i, (img, label) in enumerate(train_dataloaders):
            ...
            try:
                img_gesture, label_gesture = next(ntu_dataloader_iter)
            except StopIteration:
                ntu_dataloader_iter = iter(gesture_dataloader)
                img_gesture, label_gesture = next(ntu_dataloader_iter)

            img_gesture, label_gesture = img_gesture.cuda(), label_gesture.cuda()
            heads, _ = net(img) #Hand pose estmimation batch
            net = net.eval() # <-----
            #with torch.no_grad(): # <-----
            _, pred_gestures = net(img_gesture)
            net = net.train() # <-----
            optimizer.zero_grad()
            Cls_loss, Reg_loss = criterion(heads, label)
            Gesture_loss = criterion_2(pred_gestures, label_gesture)
            _, predicted_gesture = torch.max(pred_gestures.data, 1)
            loss = 1 * Cls_loss + Reg_loss * RegLossFactor + 0.01 * Gesture_loss
            loss.backward()
            optimizer.step()

            torch.cuda.synchronize()
            total_samples = total_samples + len(label_gesture)

            training_running_corrects = training_running_corrects + (predicted_gesture == label_gesture).sum().item()
            ...
            Gesture_loss_add = Gesture_loss_add + (Gesture_loss.item()) * len(img_gesture)
            # printing loss info
            if i % 10 == 0:
        ...
        if (epoch % 1 == 0):
            ...
            for i, (img, label) in tqdm(enumerate(test_dataloaders)):
                with torch.no_grad():
                    img, label = img.cuda(), label.cuda()
                    heads, heads_classification_ntu = net(img)
